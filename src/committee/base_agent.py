...
class BaseCommitteeAgent(ABC):
    ...
    def _format_case_brief(self, case_file: dict) -> str:
        ...
        brief_lines = [
            ...
            f"  Anomaly Score : {ml.get('anomaly_score', 'N/A')}",
            f"  Is Outlier    : {ml.get('is_outlier', 'N/A')}",
            "",
            "--- NETWORK / GRAPH ANALYSIS ---",
            f"  Betweenness Centrality       : {network.get('betweenness_centrality', 'N/A')}",
            f"  Is Hub                       : {network.get('is_hub', 'N/A')}",
            f"  Community Size               : {network.get('community_size', 'N/A')}",
            f"  Connected Flagged Accounts   : {network.get('connected_flagged_accounts', 'N/A')}",
            "",
            "--- CUSTOMER PROFILE ---",
